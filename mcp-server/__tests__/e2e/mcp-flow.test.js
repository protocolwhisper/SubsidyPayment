...
vi.mock('../../src/backend-client.ts', async () => {
    const actual = await vi.importActual('../../src/backend-client.ts');
    class BackendClient {
...
describe('MCP E2E flow (task 9.3)', () => {
    ...
    it('runs search -> auth -> task details -> complete task -> run service through tool handlers', async () => {
        ...
        const run = await mocked.registrations.get('run_service').handler({ service: 'design', input: 'generate logo', session_token: auth._meta.session_token }, context);
