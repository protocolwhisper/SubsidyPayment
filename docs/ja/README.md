...
### タスクフィードバック契約（`complete_task`）

`complete_task` 呼び出しでは、後方互換を維持するため、タスク証跡を `details` に JSON 文字列として格納します。
...
**`details` 内のプロダクトフィードバック項目（現行UI契約）**
- `product_link`: タスク対象プロダクトのURL
- `feedback_rating`: 1〜5 の星評価
- `feedback_tags`: 選択タグ（カンマ区切り）
- `feedback_reason`: タグ選択理由の自由記述
...
```json
{
  ...
  "details": "{\"email\":\"you@example.com\",\"feedback_rating\":\"4\",\"feedback_tags\":\"Cost, Usability\",\"feedback_reason\":\"オンボーディング文言が曖昧なので、セットアップ例を追加してほしいです。\",\"product_link\":\"https://example.com/product\"}",
  "consent": {
