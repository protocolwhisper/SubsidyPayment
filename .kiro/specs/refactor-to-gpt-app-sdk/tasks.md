...
- [ ] E2E フロー（サービス検索→ユーザー認証→タスク取得→タスク完了→サービス実行）を MCP プロトコル経由で実行するテストを作成する。MCP Inspector またはプログラマティックな MCP クライアントを使用する。UIウィジェットの手動検証手順（ngrok + ChatGPT 開発者モードでの確認項目一覧）を `mcp-server/docs/manual-testing.md` に文書化する。

### タスク9.4*

- [ ] Rust バックエンドの `cargo test` を実行し、既存テストが全て通過することを確認する。CORS 変更（タスク8）による回帰がないことを検証する。
