...
- [x] `render.yaml` に MCP サーバーサービス（`snapfuel-mcp`）の定義を追加する。ビルドコマンド（`cd mcp-server && npm ci && npm run build`）、起動コマンド（`cd mcp-server && npm start`）、環境変数（`RUST_BACKEND_URL`, `MCP_INTERNAL_API_KEY`, `AUTH0_DOMAIN`, `AUTH0_AUDIENCE`, `PUBLIC_URL`, `PORT`）を設定する。
